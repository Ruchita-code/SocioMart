...
const Managelisting = () => {
  ...
  const navigate = useNavigate()
    ...
    const niches = ['lifestyle', 'fitness','food','travel','tech','gaming','fashion','beauty','business','education','entertaiment','music','art','sports','health','finance','other'];

    const ageRange = ['13-17years','18-24 years','25-34 years','35-44 years','45-54 years','55+ yearns','Mixed ages'];
    ...
    //get listing data for edit if 'id' is proided (edit mmode)
    useEffect(() =>{
      if(!id) return;
    ...
    <div className='min-h-screen py-8'>
      <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8'>
        ...
        <p className='text-gray-600 mt-2'>
          {isEditing ? 'Update your existing account listing' : 'craete a mock listing to dispaly your account info'}         
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
        ...
        <Section title= 'Account Metrics'>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
            ...
            <InputField label='Follower Count *' type='number' min={0} value={formData.followers_count} placeholder='10000' onChange={(v) => handleInputChange('followers_count',v)} required={true} />
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
            ...
            <SelectField label= 'Primary audience Age Range* 'options={ageRange} value={formData.age_range} onChange={(v) => handleInputChange('age_range', v)} required={true} />
          </div>
        ...
        <Section title= 'Screenshots & Proof'>
          <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
            ...
            <label htmlFor="images" className='px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 curssor-pointer'>Choose Files</label>
            <p className='text-sm text-gary-500 mt-2'>Upload Screenshots or proof of account analytics</p>
          </div>
        ...
        <div className='flex justify-end gap-3 text-sm'>
          <button onClick={() => navigate(-1)} type='button' className='px-6 py-2 border border-gary-300 rounded-lg hover:bg-gray-50 transition-colors'>
            Cancel
          </button>
...
const TextareaField = ({ label, value, onChange, required = false }) => ( 
  <div>
    <label className='block text-sm font-medium text-gary-700 mb-2'>{label}</label>
